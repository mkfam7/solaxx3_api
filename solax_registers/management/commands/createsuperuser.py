...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            ...
            dest="overwrite",
            help=(
                "Tells Django to overwrite the username if one is found with the"
                "provided name."
            ),
        )
        parser.add_argument(
    ...
    def handle(self, *args, **options):
        ...
        try:
            if options["interactive"]:
                ...
                if username:
                    error_msg = self._validate_username(
                        username, verbose_field_name, database, options["overwrite"]
                    )
                    if error_msg:
            ...
            DoesNotExist = self.UserModel.DoesNotExist

            if options["overwrite"]:
                try:
                    db_manager.get(username=username).delete()
                except DoesNotExist:
                    pass
    ...
    def _validate_username(self, username, verbose_field_name, database, overwrite):
